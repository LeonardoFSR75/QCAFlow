...
fuzzy-set Qualitative Comparative Analysis (fsQCA) [@ragin2008]. Delivered as a
...
[@ragin2008], truth table generation across all 2ᴺ Boolean configurations with
...
for the negation of the outcome to expose causal asymmetry [@schneider2012;
...
Qualitative Comparative Analysis [@ragin1987; @ragin2000; @ragin2008] has grown
substantially in the social sciences and business administration [@greckhamer2018;
@fiss2011; @woodside2013]. Yet researchers face a persistent barrier: accessible,
free, and ready-to-use software. The reference implementation, *fs/QCA 2.0*
[@ragin2006], runs only on Windows and has not been updated since 2017. The `QCA`
package for R [@dusa2019] and the `SetMethods` package require programming
proficiency. Tosmana offers a GUI but is limited to crisp-set designs with few
conditions. None of these tools provide a Portuguese-language interface, integrated
instructional content, or export of results in publication-ready formats.
...
**Truth table generation.** For *N* conditions (3–8), the tool enumerates all 2ᴺ
configurations. Cases are assigned to the configuration in which their set-theoretic
membership—the minimum over conditions of *x* (presence) or 1−*x* (absence)—exceeds
0.5. For each row it computes sufficiency consistency [Σmin(Mᵢ,Yᵢ) / ΣMᵢ], PRI
...
(RoN) [@schneider2012] are computed. Conditions meeting the conventional consistency
threshold (≥ 0.90) are flagged as necessary, and a low RoN warns of potentially
trivial necessity.
...
expectations. Conditions appearing in both the parsimonious and intermediate
solutions are marked as *core* and the remainder as *peripheral*, following
[@fiss2011].

**Causal asymmetry and robustness.** Because set-theoretic explanations are
asymmetric [@woodside2013; @schneider2012], the tool separately derives a
conservative solution for the negation of the outcome (~Y) and flags any row coded
as sufficient for both Y and ~Y. Robustness checks re-run the analysis across
...
Fiss-style configuration chart (● present, ⊗ absent; large = core, small =
...
The author thanks the Programa de Mestrado Profissional em Administração of UNOESC
